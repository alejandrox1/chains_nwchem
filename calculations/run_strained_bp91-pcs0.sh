...
#-      bash run_strained_bp91-pcs0.sh -N 1 -np 16 -c 2 -d p -s 3.0 
#-      bash run_strained_bp91-pcs0.sh -N numbernodes -np numberoftasks -c chainlength -d (p || n) -s strain(percentage)
#-
...
echo "bash run_strained_bp91-pcs0.sh -N $nodes -np $procs -c $length -d $straindir -s $strain" > README
...
case "$flag" in
		...
		cp ${runfiles}/raman_run_strained_bp91-pcs0.sh .
		cp ${runfiles}/restart_raman.sh .
		cp ${runfiles}/strained_bp91-pcs0_acetylene.nw .
...
if [ "$length" -ne "2" ]; then
	...
	lastC=$(( $numatoms - 1 ))
	sed -i "s/atom 1 2.*/atom 1 ${lastC}/" strained_bp91-pcs0_acetylene.nw
fi
sed -i "s/monomer/${straindir}${strain}_c${length}/g" strained_bp91-pcs0_acetylene.nw
sed -i "s/monomer/${straindir}${strain}_c${length}/g" acetylene_restart.nw


sbatch -A $xsede -p $partition -N $nodes -n $procs -J ${straindir}c${length} raman_run_strained_bp91-pcs0.sh $procs > submit.txt
