...
#~
#~ NOTE: It is impossible to create directories with '/', hence this should be run manually.
#~
...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	# Respect the max job limit!	
	joblim=50
	currjobs=`squeue -u alarcj | wc -l`
	if [ "${currjobs}" -le "$joblim" ]; then
		...
		if [ ! -d "$set" ]; then
			...
			# run calculation
			sbatch -A TG-TRA140037 -p normal -N 1 -n 8 calc.sh 8 
